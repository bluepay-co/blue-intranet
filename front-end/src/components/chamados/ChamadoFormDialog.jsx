...
/**
 * Dialog de abertura/edição de chamado.
 * Na edição (chamadoEditando) só título e descrição são editáveis, e somente
 * enquanto o status for "Em Aberto" (a regra também é garantida no backend).
 *
 * @param {{ aberto: boolean, onFechar: () => void, chamadoEditando?: object|null, onSalvo?: () => void }} props
 */
export default function ChamadoFormDialog({ aberto, onFechar, chamadoEditando, onSalvo }) {
  ...
  return (
    <Dialog open={aberto} onOpenChange={onFechar}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{ehEdicao ? 'Editar chamado' : 'Abrir chamado'}</DialogTitle>
          <DialogDescription>
            {ehEdicao
              ? 'Você pode ajustar o título e a descrição enquanto a T.I. não iniciar o atendimento.'
              : 'Descreva o problema com o máximo de detalhes para agilizar o suporte.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              ...
              placeholder="Ex.: Impressora do RH não puxa papel"
              maxLength={200}
            />
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              rows={6}
              placeholder="O que está acontecendo? Desde quando? Já tentou algo?"
