...
import { getMembrosEquipeGerente, getPessoalEquipeGerente } from '@/api/modules/gerente'
...
import {
  ...
  ChevronLeft, ChevronRight, Target, UserRound,
} from 'lucide-react'
...
/** Comparativo Ano × Ano — receita mensal do ano atual vs. anterior. */
      ...
      <CardContent>
        <ChartContainer className="h-64">
          <BarChart data={historico} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="gradBarEquipePessoal" x1="0" y1="0" x2="0" y2="1">
...
/**
 * Dashboard pessoal de um funcionário específico, visto pelo gerente — réplica
 * independente de `DashboardPessoal.jsx` (dados vêm de `getPessoalEquipeGerente`
 * em vez de `getMeuResumo`/`getTopClientes`). Propositalmente não compartilha
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      if (e.response?.status === 404)
        setErro('Este funcionário não possui métricas cadastradas no banco de produção.')
      else if (e.response?.status === 403)
        setErro('Funcionário fora da sua equipe.')
      else
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Dashboard pessoal</p>
        </div>
              ...
              <div className="space-y-6">
                {anual && anual.meta > 0 ? (
                  ...
                  <div className="flex items-center gap-2 rounded-lg border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
                    <AlertCircle className="size-4 shrink-0" />
                    Sem meta anual cadastrada para este funcionário.
                  </div>
                )}
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Dashboard pessoal de cada funcionário da equipe"
      />
        ...
        <div className="flex flex-wrap gap-2">
          {membros.map((m) => (
            <button
              key={m.id}
              type="button"
              onClick={() => setVendedorSelecionado(m.id === vendedorSelecionado?.id ? null : m)}
              className={cn(
                'flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-colors',
                vendedorSelecionado?.id === m.id
      ...
      {vendedorSelecionado ? (
        ...
        !carregandoMembros && (
          <p className="py-8 text-center text-sm text-muted-foreground">
            Selecione um funcionário acima para ver o dashboard pessoal dele.
          </p>
        )
      )}
    </div>
